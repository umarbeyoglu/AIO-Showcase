...
class CartScreenState extends State<CartScreen> {
  ...
  Future _loadData() async {
    FetchCartItems(page)..then((result){
      setState(() {
                                  ...
                                  itemBuilder: (context,index){
                                    return Column(
                                      children: [
                                        ...
                                        Text(posts[index].price.toString()),
                                        InkWell(
                                          child: Text('Choices'),
                                          onTap:(){
                                            List<ArticleChoice> cichoice = [];
                                            if(posts[index].type == 'UP'){
                                          for(int i=0;i<posts[index].choices.length;i++){
                                            ArticleChoice choice = ArticleChoice(price: posts[index].choices[i].price.toString(),
                                        item:posts[index].choices[i].item);
                                            cichoice.add(choice);}
                                              Navigator.push(
                                                context,
                                                MaterialPageRoute(
                                                    builder: (_) => UserProductChoiceScreen(
                                                      user:user,product: Article(choices:cichoice),
                                                    )
                                                ),
                                              );
                                            }
                                        ...
                                        InkWell(
                                          ...
                                          for(int i=0;i<posts.length;i++){
                                            double price = 0;
                                            price = price + posts[i].price.toDouble();
                                            for(int y=0;y<posts[i].choices.length;y++){
                                              price = price + posts[i].choices[y].price.toDouble();
                                            }
                                          }
                                          },
                                        ),
