...
const SERVER_IP = 'https://aio_ui.net';
...
class ReservationScheduleScreen extends StatefulWidget {
  ...
  ReservationScheduleScreen(
      {Key key, this.user, this.reservation, this.calendarowner})
      : super(key: key);
  @override
  ReservationScheduleScreenS createState() => new ReservationScheduleScreenS(
      user: user, reservation: reservation, calendarowner: calendarowner);
}
...
class ReservationScheduleScreenS extends State<ReservationScheduleScreen> {
  ...
  bool isdayactive(String date) {
    for (int i = 0; i < scheduleddays.length; i++) {
      if (scheduleddays[i] == date) {
        return false;
      }
    }
    for (int i = 0; i < deactivateddays.length; i++) {
      if (deactivateddays[i] == date) {
        return false;
      }
    }
    return true;
  ...
  void initState() {
    for (int i = 0;
        i < calendarowner.reservationdeactivationmonths.length;
        i++) {
      deactivateddays.addAll(datesinbetween(
          calendarowner.reservationdeactivationmonths[i].startdate,
    ...
    for (int i = 0; i < calendarowner.reservationschedules.length; i++) {
      if (calendarowner.reservationschedules[i].reservation == reservation.id) {
        scheduleddays.addAll(datesinbetween(
            calendarowner.reservationschedules[i].startdate,
    ...
    for (final item in scheduleddays) {
      _markedDateMap.add(
          DateTime(
              ...
              int.parse(item.substring(5, 7)),
              int.parse(item.substring(8, 10))),
          Event(
            date: DateTime(
                int.parse(item.substring(0, 4)),
                int.parse(item.substring(5, 7)),
      ...
      onDayPressed: (date, events) {
        setState(() {
          ...
          if (enddate != '') {
            ...
            if (year == yearnew && monthnew > month) {
              error = 'ERROR/HATA';
              if (startdate != '' && enddate != '' && isconflictingdays()) {
                startdate = '';
                enddate = '';
            ...
            if (year == year && monthnew == month && daynew > day) {
              error = 'ERROR/HATA';
              if (startdate != '' && enddate != '' && isconflictingdays()) {
                startdate = '';
                enddate = '';
...
class ReservationScheduleScreen2 extends StatefulWidget {
  ...
  ReservationScheduleScreen2({
    Key key,
    this.user,
  }) : super(key: key);
  @override
  ReservationScheduleScreen2S createState() => new ReservationScheduleScreen2S(
        user: user,
      );
}
    ...
    return Scaffold(
      body: ListView.builder(
          ...
          itemBuilder: (context, index) {
            return doesitembelongtoschedule(user.reservationschedules[index])
                ? FutureBuilder<User>(
                              ...
                              child: ListTile(
                                  ...
                                  title: Column(
                                    children: [
                                      ...
                                      Text(
                                        '''$startdatest : ${user.reservationschedules[index].startdate.toString()} ''',
                                        style: TextStyle(
                                          fontSize: MediaQuery.of(context)
                                                  .size
                                                  .height *
                                      ...
                                      Text(
                                        '''$enddatest: ${user.reservationschedules[index].enddate.toString()} ''',
                                        style: TextStyle(
                                          fontSize: MediaQuery.of(context)
                                                  .size
                                                  .height *
                                  ...
                                  trailing: IconButton(
                                    ...
                                    onPressed: () {
                                      RemoveReservationSchedule(
                                          user.reservationschedules[index].id);
                                      Future.delayed(
                                          Duration(seconds: 1),
...
class ReservationDeactDayScreen extends StatefulWidget {
  final User user;
  ReservationDeactDayScreen({
    Key key,
    this.user,
  }) : super(key: key);
  @override
  ReservationDeactDayScreenS createState() => new ReservationDeactDayScreenS(
        user: user,
      );
}
            ...
            children: <Widget>[
              ...
              if (error == '')
                InkWell(
                  onTap: () {
                    if (startdate != '' && enddate != '') {
                      CreateReservationDeactivationMonth(
