...
import '../Calendar_Screen/calendar_create_time_screen.dart';
...
class StatusScreenState extends State<StatusScreen> {
  ...
  Future _loadData() async {
    ...
    setState(() {
      page = page + 1;
      for (final item2 in postsinit) isLoading = false;
      return;
    });
  }
                              ...
                              itemBuilder: (context, index) {
                                return Column(
                                  children: [
                                    ListTile(
                                      title: Text(
                                        '''$daysst: ${correctdatedays(posts[index].dates).toString()} \n'''
                                        '''\n'''
...
class StatusDateChooseScreen extends StatefulWidget {
  ...
  const StatusDateChooseScreen({Key key, this.user, this.calendarowner})
      : super(key: key);
  @override
  StatusDateChooseScreenState createState() =>
      StatusDateChooseScreenState(user: user, calendarowner: calendarowner);
}
                ...
                child: ElevatedButton(
                  ...
                  onPressed: () {
                    ...
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (_) => StatusTimeChooseScreen(
                          user: user,
                        ),
                      ),
                    );
...
class StatusTimeChooseScreen extends StatefulWidget {
  ...
  @override
  StatusTimeChooseScreenState createState() =>
      StatusTimeChooseScreenState(user: user);
}
...
class MeetingScheduleScreen extends StatefulWidget {
  ...
  @override
  MeetingScheduleScreenS createState() => new MeetingScheduleScreenS(
        user: user,
      );
}
...
class ScheduleScreen extends StatefulWidget {
  ...
  const ScheduleScreen({Key key, this.user, this.day, this.mnyinit})
      : super(key: key);
  @override
  ScheduleScreenState createState() =>
      ScheduleScreenState(user: user, mnyinit: mnyinit, day: day);
}
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: ListView.builder(
          ...
          controller: _scrollController,
          itemBuilder: (context, index) {
            return doesitembelongtoschedule(user.meetingschedules[index])
                ? FutureBuilder<User>(
                              ...
                              child: ListTile(
                                  title: Column(
                                    children: [
                                      ...
                                      Text(
                                        '''${user.meetingschedules[index].date.toString()} ''',
                                        style: TextStyle(
                                          fontSize: MediaQuery.of(context)
                                                  .size
                                                  .height *
                                      ...
                                      Text(
                                        '''$timest: ${user.meetingschedules[index].time.toString()} ''',
                                        style: TextStyle(
                                          fontSize: MediaQuery.of(context)
                                                  .size
                                                  .height *
                                  ...
                                  trailing: IconButton(
                                    ...
                                    onPressed: () {
                                      RemoveMeetingSchedule(
                                          user.meetingschedules[index].id);
                                      Future.delayed(
                                          Duration(seconds: 1),
                                          () => Navigator.pushReplacement(
                                              context,
...
class MeetingScheduleDetailsScreen extends StatefulWidget {
  ...
  final User user;
  final User client;
  const MeetingScheduleDetailsScreen({
    Key key,
    this.client,
    this.calendaritem,
    this.user,
  ...
  @override
  MeetingScheduleDetailsScreenState createState() =>
      MeetingScheduleDetailsScreenState(
          user: user, client: client, calendaritem: calendaritem);
}

class MeetingScheduleDetailsScreenState
    extends State<MeetingScheduleDetailsScreen> {
  final MeetingSchedule calendaritem;
  final User user;
  final User client;
  MeetingScheduleDetailsScreenState({
...
class CalendarMeet extends StatefulWidget {
  ...
  final bool issearch;
  final User calendarowner;
  CalendarMeet({Key key, this.user, this.issearch, this.calendarowner})
      : super(key: key);
  @override
  CalendarMeetS createState() => new CalendarMeetS(
...
class TimeRequestScreen extends StatefulWidget {
  ...
  const TimeRequestScreen(
      ...
      : super(key: key);
  @override
  TimeRequestScreenState createState() => TimeRequestScreenState(
      user: user,
      issearch: issearch,
      link: link,
      calendarowner: calendarowner,
...
class TimeRequestScreenState extends State<TimeRequestScreen> {
    ...
    isuserfollowed(user.id, calendarowner.id)
      ..then((result) {
        if (user.shouldshowuserinfo(calendarowner, result) == 'followed') {
          setState(() {
            print('hereb');
            fetchPostsfuture = FetchFollowedCalendarTime(
                http.Client(), calendarowner.id, page);
          });
          return;
        }
        if (user.shouldshowuserinfo(calendarowner, result) == 'public') {
          setState(() {
            print('herec');
            fetchPostsfuture =
                FetchPublicCalendarTime(http.Client(), calendarowner.id, page);
          });
          return;
        }
      });
  ...
  Future _loadData() async {
    if (calendarowner.id == user.id) {
      FetchOwnCalendarTime(http.Client(), user.id, page)
        ..then((result) {
          setState(() {
    ...
    isuserfollowed(user.id, calendarowner.id)
      ..then((result) {
        if (user.shouldshowuserinfo(calendarowner, result) == 'followed') {
          FetchFollowedCalendarTime(http.Client(), calendarowner.id, page)
            ..then((result2) {
              setState(() {
  ...
  bool canitembechosen(String calendartime) {
    for (int i = 0; i < calendarowner.meetingschedules.length; i++) {
      String mnyincalendar =
          calendarowner.meetingschedules[i].date.substring(0, 7);
      String dayincalendar =
          calendarowner.meetingschedules[i].date.substring(8, 10);
          ...
          IconButton(
            ...
            onPressed: () {
              Navigator.of(context)
                  .push(MaterialPageRoute(
                      builder: (context) =>
                          CreateMeetingTimeScreen(user: calendarowner)))
                  .then((value) {
                Future.delayed(
                    Duration(seconds: 1),
                    () => Navigator.pushReplacement(
