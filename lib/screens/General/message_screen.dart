...
class MessagingHomeScreen extends StatefulWidget {
  ...
  final User user;
  @override
  MessagingHomeScreenState createState() =>
      MessagingHomeScreenState(user: user);
}
...
class MessagingHomeScreenState extends State<MessagingHomeScreen> {
  ...
  void didChangeDependencies() {
    ...
    timer = Timer.periodic(
        ...
        (Timer t) => FetchUserMC()
          ..then((value) {
            if (start == false &&
                (value > messagecount || value < messagecount)) {
              messagecount = value;
              messagecheck();
            }
            if (start) {
  ...
  void startcheck() {
    ...
    for (int i = 0; i < allmessages.length; i++) {
      for (int r = 0; r < user.recentchats2.length; r++) {
        if (user.recentchats2[r].guest == allmessages[i].authorinit ||
          ...
          user.recentchats2[r].messages.add(allmessages[i]);
          getout = true;
        }
      }
      ...
      if (allmessages[i].authorinit == user.id) {
        ...
        test.messages.add(allmessages[i]);
        user.recentchats2.add(test);
        getout = true;
      }
      if (getout) {
        getout = false;
    ...
    for (int r = 0; r < user.recentchats2.length; r++) {
      user.recentchats2[r].timestamp =
          user.recentchats2[r].messages.last.timestamp;
  ...
  void messagecheck() async {
    ...
    final response = await http.get(
      Uri.parse("$SERVER_IP/api/usergetmessages/?format=json"),
                    ...
                    child: Container(
                      child: ListView.builder(
                        ...
                        itemBuilder: (BuildContext context, int index) {
                          user.recentchats2.sort(
                              (b, a) => a.timestamp.compareTo(b.timestamp));
                          return FutureBuilder<User>(
                              future: FetchUserPrivate(
                                  user.recentchats2[index].guest),
      ...
      children: <Widget>[
        InkWell(
          child: Container(
            ...
            padding:
                const EdgeInsets.only(left: 18, right: 0, top: 12, bottom: 12),
          ...
          IconButton(
            ...
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (_) => MessagingHomeScreen(user: user),
                ),
              );
            },
          ),
...
CreateMessage(
    String author, String profile, String article, String content) async {
  try {
    ...
    await Dio().post("$SERVER_IP/api/usermessages/",
        data: formData,
...
class ChatSearchScreen extends StatefulWidget {
  ...
  @override
  ChatSearchScreenState createState() =>
      ChatSearchScreenState(user: user, key: key);
}
            ...
            child: Column(
              children: [
                ListTile(
                  ...
                  onTap: () {
                    returnfullmessages(user, _usersForDisplay[index].id,
                        RecentChat(guest: _usersForDisplay[index].id));
                    user.relevantmessages = [];
                    Navigator.push(
                      context,
                      MaterialPageRoute(
