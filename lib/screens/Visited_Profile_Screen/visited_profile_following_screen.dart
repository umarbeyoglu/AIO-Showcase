...
import 'package:flutter_food_delivery_app/models/User_Model/user_follow_model.dart';
...
class VisitedFollowingScreen extends StatefulWidget {
  ...
  const VisitedFollowingScreen({Key key, this.user, this.visiteduser})
      : super(key: key);

  @override
  VisitedFollowingScreenState createState() =>
      VisitedFollowingScreenState(user: user, visiteduser: visiteduser);
}
...
class VisitedFollowingScreenState extends State<VisitedFollowingScreen> {
    ...
    isuserfollowed(user.id, visiteduser.id)
      ..then((result) {
        if (user.shouldshowuserinfo(visiteduser, result) == 'followed') {
          setState(() {
            fetchPostsfuture = FetchFollowedUserFollowings(
        ...
        if (user.shouldshowuserinfo(visiteduser, result) == 'public') {
          setState(() {
            fetchPostsfuture =
                FetchPublicUserFollowings(http.Client(), visiteduser.id, page);
          });
          return;
        }
      });
  ...
  Future _loadData() async {
    isuserfollowed(user.id, visiteduser.id)
      ..then((result) {
        if (user.shouldshowuserinfo(visiteduser, result) == 'followed') {
          FetchFollowedUserFollowings(http.Client(), visiteduser.id, page)
            ..then((result2) {
              setState(() {
