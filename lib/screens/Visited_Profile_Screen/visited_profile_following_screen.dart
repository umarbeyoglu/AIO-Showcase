...
import 'package:untitled/models/User_Model/user_follow_model.dart';
...
class VisitedFollowingScreen extends StatefulWidget {
  final User user;
  final User visiteduser;
  const VisitedFollowingScreen({Key key,this.user,this.visiteduser}) : super(key : key);

  @override
  VisitedFollowingScreenState createState() => VisitedFollowingScreenState(user: user,visiteduser: visiteduser );
}

class VisitedFollowingScreenState extends State<VisitedFollowingScreen> {
  final User user;
  final User visiteduser;
  VisitedFollowingScreenState({Key key,this.user,this.visiteduser});
  ...
  void initState() {
    ...
    setState(() {
      fetchPostsfuture = FetchFollowedUserFollowings(http.Client(),visiteduser.id,page);
    });   return;

      }
      if(user.shouldshowuserinfo(visiteduser,result) == 'public'){
   setState(() {
     fetchPostsfuture = FetchPublicUserFollowings(http.Client(),visiteduser.id,page);
   });   return;
  ...
  Future _loadData() async {
    isuserfollowed(user.id,visiteduser.id)..then((result){
      if(user.shouldshowuserinfo(visiteduser,result) == 'followed'){
        FetchFollowedUserFollowings(http.Client(),visiteduser.id,page)..then((result2){
          setState(() {
      ...
      if(user.shouldshowuserinfo(visiteduser,result) == 'public'){
        FetchPublicUserFollowings(http.Client(),visiteduser.id,page)..then((result2){
          setState(() {
            for(final item2 in result2)
              posts.add(item2);
            isLoading = false;
