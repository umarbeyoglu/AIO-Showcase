...
class VisitedProfileFeedScreen extends StatefulWidget {
  ...
  const VisitedProfileFeedScreen({Key key, this.visiteduser, this.user})
      : super(key: key);

  @override
  VisitedProfileFeedScreenState createState() =>
      VisitedProfileFeedScreenState(user: user, visiteduser: visiteduser);
}
    ...
    isuserfollowed(user.id, visiteduser.id)
      ..then((result) {
        if (user.shouldshowuserinfo(visiteduser, result) == 'followed') {
          FetchVisitedProfileFollowedArticles(
              http.Client(), visiteduser.id, page)
            ..then((result2) {
              setState(() {
                for (final item in result2) posts.add(item);
                isLoading = false;
              });
              return;
            });
        ...
        if (user.shouldshowuserinfo(visiteduser, result) == 'public') {
          FetchVisitedProfilePublicArticles(http.Client(), visiteduser.id, page)
            ..then((result2) {
              setState(() {
                for (final item in result2) posts.add(item);
                isLoading = false;
