...
class VisitedFollowersScreen extends StatefulWidget {
  ...
  const VisitedFollowersScreen({Key key,this.user,this.visiteduser}) : super(key : key);

  @override
  VisitedFollowersScreenState createState() => VisitedFollowersScreenState(user: user,visiteduser: visiteduser );
}
...
class VisitedFollowersScreenState extends State<VisitedFollowersScreen> {
  ...
  void initState() {
    ...
    isuserfollowed(user.id,visiteduser.id)..then((result){
      if(user.shouldshowuserinfo(visiteduser,result) == 'followed'){
        setState(() {
          fetchPostsfuture = FetchFollowedUserFolloweds(http.Client(),visiteduser.id,page);
        });   return;

      }
      if(user.shouldshowuserinfo(visiteduser,result) == 'public'){
        setState(() {
          fetchPostsfuture = FetchPublicUserFolloweds(http.Client(),visiteduser.id,page);
        });   return;
  ...
  Future _loadData() async {
    ...
    isuserfollowed(user.id,visiteduser.id)..then((result){
      if(user.shouldshowuserinfo(visiteduser,result) == 'followed'){
        FetchFollowedUserFolloweds(http.Client(),visiteduser.id,page)..then((result2){
          setState(() {
      ...
      if(user.shouldshowuserinfo(visiteduser,result) == 'public'){
        FetchPublicUserFolloweds(http.Client(),visiteduser.id,page)..then((result2){
          setState(() {
            for(final item2 in result2)
              posts.add(item2);
            isLoading = false;
