import 'package:Welpie/screens/First_Time_Screen/first_time_name_screen.dart';
...
class RegisterScreen extends StatefulWidget {
  ...
  const RegisterScreen({Key key, this.user,this.issubprofile}) : super(key: key);
  @override
  RegisterScreenState createState() => RegisterScreenState(user:user,issubprofile:issubprofile);
}
...
class RegisterScreenState extends State<RegisterScreen> with SingleTickerProviderStateMixin{
  ...
  navigatelogin(User loginuser,String subprofileusername){
    ...
    Future<User> GetUser(String token) async {
      ...
      if (response.statusCode == 200) {
        ...
        Navigator.of(context).push(MaterialPageRoute(builder: (context) => FirstTimeNameScreen(user:loginuser)));
    ...
    Future<String> signIn(String email, String password) async {
      ...
      WidgetsFlutterBinding.ensureInitialized();
      final http.Response response = await http.post(Uri.parse("$SERVER_IP/auth/"),
  ...
  @override
  void initState() {
    RegisteringCheck().then((value) {setState(() {_users.addAll(value);_usersForDisplay = _users;});});
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    init();
    return createpersonprofiletype == 'P' ? Scaffold(
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    Padding(
                      ...
                      child: Row(
                        ...
                        children: [
                          ElevatedButton(
                            child: Text(userst, style:TextStyle(fontSize:MediaQuery.of(context).size.height*0.02,),),
                          ...
                          ElevatedButton(
                            child: Text(businessst, style:TextStyle(fontSize:MediaQuery.of(context).size.height*0.02,),),
                    ...
                    issubprofile ? Container(height: 0,width: 0,) : Padding(
                    ...
                    Text(doesemailexist(_usersForDisplay) ? emailexistsst : '',style: TextStyle(color: Colors.red),),
                    Padding(
                      padding: EdgeInsets.only(left: MediaQuery
                          .of(context)
                          .size
                    ...
                    Text(doesusernameexist(_usersForDisplay) ? usernameexistsst : '',style: TextStyle(color: Colors.red),),
                    Padding(
                      padding: EdgeInsets.only(left: MediaQuery
                          .of(context)
                          .size
                 ...
                 CheckboxListTile(
                      title:  InkWell(onTap:(){
                        ...
                        },child: Text(termsandconditionsst,style: TextStyle(color:colordtmaintwo,fontWeight:  FontWeight.w400)),),
                      ...
                      child: ElevatedButton(
                        ...
                        onPressed: () {
                          ...
                          setState(() {
                            ...
                            Future<User> CreateUser(String email,String password,String username,String firstname,String lastname,
                                String location,String gender,String profiletype,String phone,String about,File image,String tags) async {
                              ...
                              if (response.statusCode == 201) {
                                ...
                                final http.Response response2 = await http.post(Uri.parse("$SERVER_IP/auth/"),
                                  body: jsonEncode(<String, String>{
                                    'username':email,
                                    'password':password,
                                  }),
                            ...
                            Future<User> CreateUserSub(String email,String password,String username,String firstname,String lastname,
                                String location,String gender,String profiletype,String phone,String about,File image,String tags) async {
                              final http.Response response = await http.post(
                                Uri.parse("$SERVER_IP/api/register/",),
                                headers: <String, String>{
                                  'Content-Type': 'application/json',"Authorization" : "Token ${globaltoken}",
                                },
                                body: jsonEncode(<String, dynamic>{
                                  'email' :'$username.${user.email}' , // done
                                  'username' : username, // done
                                  'password' : password, // done
                                  'fullname' : '-', // done
                                  'phone_number' : '0', // done
                              ...
                              if (response.statusCode == 201) {
                                ...
                                if (response2.statusCode == 200) {
                                  ...
                                  Future.delayed(new Duration(seconds:2), () {
                                    ...
                                    navigatelogin(uniuser,'$username.${user.email}');
                                  });
                                }
                            ...
                            issubprofile ? (_email.text = '') : (_email.text == '' ? _showError(validemailst) : signupcheck = signupcheck + 1);
                            _password.text == '' ? _showError(validps2st) : signupcheck = signupcheck + 1;
                            _username.text == '' ? _showError(validusernamest) : signupcheck = signupcheck + 1;
                            termsandconditions == false ? _showError(termsandconditionserrorst) : signupcheck = signupcheck + 1;

                            if(emailexists == false){signupcheck = signupcheck + 1;}
                            if(usernameexists == false){signupcheck = signupcheck + 1;}
                            signupcheck2 = signupcheck ;
                            signupcheck = 0;
                            // ignore: unnecessary_statements
                            issubprofile ? (signupcheck2 == 5 ? CreateUserSub(_email.text,_password.text,_username.text,_firstname.text,_lastname.text,_location.text,null,createpersonprofiletype,_phonenumber.text
                                // ignore: unnecessary_statements
                                ,_bio.text,image,tags.text) : _showError(inputerrorst)) : (signupcheck2 == 6 ? CreateUser(_email.text,_password.text,_username.text,_firstname.text,_lastname.text,_location.text,null,createpersonprofiletype,_phonenumber.text
                                ,_bio.text,image,tags.text) : _showError(inputerrorst));
                            signupcheck = 0;
                            if (errortrue == true){
                              errortrue = false;
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      ...
                      onPressed: () {
                        ...
                        setState(() {
                          Future<User> CreateUserSub(String email,String password,String username,String firstname,String lastname,
                              String location,String gender,String profiletype,String phone,String about,String businesstype,File image,String tags) async {
                            final http.Response response = await http.post(
                              Uri.parse("$SERVER_IP/api/register/",),
                              headers: <String, String>{
                                'Content-Type': 'application/json',"Authorization" : "Token ${globaltoken}",
                          ...
                          Future<User> CreateUser(String email,String password,String username,String firstname,String lastname,
                            ...
                            final http.Response response = await http.post(
                              ...
                              body: jsonEncode(<String, dynamic>{
                                'email' :issubprofile ? '$username.${user.email}' : email, // done
                                'username' : username, // done
                                'password' : password, // done
                                'fullname' : '-', // done
                                'phone_number' : '0', // done
                            ...
                            if (response.statusCode == 201) {
                              ...
                              final http.Response response2 = await http.post(Uri.parse("$SERVER_IP/auth/"),
                                body: jsonEncode(<String, String>{
                                  'username':issubprofile ? '$username.${user.email}' : email,
                                  'password':password,
                                }),
                                headers: <String, String>{
                                  'Content-Type': 'application/json',
                          ...
                          issubprofile ? (signupcheck2 == 5 ? CreateUserSub(_email.text,_password.text,_username.text,_firstname.text,_lastname.text,_location.text,null,createpersonprofiletype,_phonenumber.text
                              ,_bio.text,_businesstype.text,image,tags.text) : _showError(inputerrorst)) : (signupcheck2 == 6 ? CreateUser(_email.text,_password.text,_username.text,_firstname.text,_lastname.text,_location.text,null,createpersonprofiletype,_phonenumber.text
                              ,_bio.text,_businesstype.text,image,tags.text) : _showError(inputerrorst));
                          signupcheck = 0;
                          if (errortrue == true){
                            errortrue = false;
                            _showError(errortruest);
