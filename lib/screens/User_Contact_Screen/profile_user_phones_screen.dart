...
class UserPhoneScreen extends StatefulWidget {
  ...
  const UserPhoneScreen({Key key, this.user, this.visiteduser}) : super(key: key);

  @override
  UserPhoneScreenState createState() => UserPhoneScreenState(user: user, visiteduser: visiteduser);
}
...
class UserPhoneScreenState extends State<UserPhoneScreen> {
  ...
  void initState() {
    ...
    isuserfollowed(user.id,visiteduser.id)..then((result){
      if(user.shouldshowuserinfo(visiteduser,result) == 'followed'){
        setState(() {
          fetchPostsfuture = FetchFollowedUserPhone(http.Client(),visiteduser.id,page);
        });   return;

      }
      if(user.shouldshowuserinfo(visiteduser,result) == 'public'){
        setState(() {
          fetchPostsfuture = FetchPublicUserPhone(http.Client(),visiteduser.id,page);
        });   return;
  ...
  Future _loadData() async {
    if(visiteduser.id == user.id){
     FetchOwnUserPhone(http.Client(),user.id,page)..then((result2){
       setState(() {
    ...
    isuserfollowed(user.id,visiteduser.id)..then((result){
      if(user.shouldshowuserinfo(visiteduser,result) == 'followed'){
        FetchFollowedUserPhone(http.Client(),visiteduser.id,page)..then((result2){
          setState(() {
            for(final item2 in result2)
              posts.add(item2);
            isLoading = false;
      ...
      if(user.shouldshowuserinfo(visiteduser,result) == 'public'){
        FetchPublicUserPhone(http.Client(),visiteduser.id,page)..then((result2){
          setState(() {
            for(final item2 in result2)
              posts.add(item2);
            isLoading = false;
  ...
  UserPhoneScreenState({Key key, this.user, this.visiteduser, });
                        ...
                        ListView.builder(
                            ...
                            itemBuilder: (context,index){
                              ...
                              0.03,
                                                    ...
                                                    launch("whatsapp://send?phone=${posts[index].phone}"):
                                                    launch("https://play.google.com/store/apps/details?id=com.whatsapp&hl=en&gl=US")
                                                    )
                                                        :
                                                    launch("tel:<${posts[index].phone}>");
                                                  },),
...
class UserPhoneCreateScreen extends StatefulWidget {
  ...
  const UserPhoneCreateScreen({Key key, this.user,this.visiteduser}) : super(key: key);

  @override
  UserPhoneCreateScreenState createState() =>
      UserPhoneCreateScreenState(user: user,visiteduser:visiteduser, );
}
              ...
              child: Column(
                ...
                Padding(
                  ...
                  child: Container(
                    ...
                    CheckboxListTile(
                      title:  Text(iswhatsappst,style: TextStyle(color:colordtmaintwo,fontWeight:  FontWeight.w400)),
                      value: iswp,
                    ...
                    CheckboxListTile(
                      title:  Text(istelegramst,style: TextStyle(color:colordtmaintwo,fontWeight:  FontWeight.w400)),
                      value: iswp,
                      onChanged: (bool value) {
                        setState(() { iswp = value; });
                      },
