...
class UserLocationScreen extends StatefulWidget {
  ...
  const UserLocationScreen({Key key, this.user, this.visiteduser})
      : super(key: key);

  @override
  UserLocationScreenState createState() =>
      UserLocationScreenState(user: user, visiteduser: visiteduser);
}
...
class UserLocationScreenState extends State<UserLocationScreen> {
    ...
    isuserfollowed(user.id, visiteduser.id)
      ..then((result) {
        if (user.shouldshowuserinfo(visiteduser, result) == 'followed') {
          setState(() {
            fetchPostsfuture =
                FetchFollowedUserLocation(http.Client(), visiteduser.id, page);
          });
          return;
        }
        if (user.shouldshowuserinfo(visiteduser, result) == 'public') {
          setState(() {
            fetchPostsfuture =
                FetchPublicUserLocation(http.Client(), visiteduser.id, page);
          });
          return;
        }
      });
  ...
  Future _loadData() async {
    if (visiteduser.id == user.id) {
      FetchOwnUserLocation(http.Client(), user.id, page)
        ..then((result) {
          setState(() {
...
class UserLocationCreateScreen extends StatefulWidget {
  ...
  const UserLocationCreateScreen({Key key, this.user, this.visiteduser})
      : super(key: key);

  @override
  UserLocationCreateScreenState createState() => UserLocationCreateScreenState(
        user: user,
        visiteduser: visiteduser,
      );
}
