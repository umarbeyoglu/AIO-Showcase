...
class UserMailScreen extends StatefulWidget {
  ...
  const UserMailScreen({Key key, this.user, this.visiteduser}) : super(key: key);

  @override
  UserMailScreenState createState() => UserMailScreenState(user: user, visiteduser: visiteduser);
}
...
class UserMailScreenState extends State<UserMailScreen> {
  ...
  UserMailScreenState({Key key, this.user, this.visiteduser, });
  @override
  void initState() {
    ...
    isuserfollowed(user.id,visiteduser.id)..then((result){
      if(user.shouldshowuserinfo(visiteduser,result) == 'followed'){
        setState(() {
          fetchPostsfuture = FetchFollowedUserMail(http.Client(),visiteduser.id,page);
        });   return;

      }
      if(user.shouldshowuserinfo(visiteduser,result) == 'public'){
        setState(() {
          fetchPostsfuture = FetchPublicUserMail(http.Client(),visiteduser.id,page);
        });   return;
  ...
  Future _loadData() async {
    if(visiteduser.id == user.id){
      FetchOwnUserMail(http.Client(),user.id,page)..then((result){
        setState(() {
    ...
    isuserfollowed(user.id,visiteduser.id)..then((result){
      if(user.shouldshowuserinfo(visiteduser,result) == 'followed'){
        FetchFollowedUserMail(http.Client(),visiteduser.id,page)..then((result2){
          setState(() {
      ...
      if(user.shouldshowuserinfo(visiteduser,result) == 'public'){
        FetchPublicUserMail(http.Client(),visiteduser.id,page)..then((result2){
          setState(() {
            for(final item2 in result2)
              posts.add(item2);
            isLoading = false;
                                              ...
                                              child: Column(
                                                ...
                                                children: <Widget>[
                                                 InkWell(
                                                   ...
                                                   onTap: (){
                                                     launch("mailto:<${posts[index].mail}>");
...
class UserMailCreateScreen extends StatefulWidget {
  ...
  const UserMailCreateScreen({Key key, this.user,this.visiteduser}) : super(key: key);

  @override
  UserMailCreateScreenState createState() =>
      UserMailCreateScreenState(user: user,visiteduser:visiteduser, );
}
