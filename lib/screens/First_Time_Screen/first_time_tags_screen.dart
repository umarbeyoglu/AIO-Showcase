...
class FirstTimeTagsScreen extends StatefulWidget {
  final User user;
  const FirstTimeTagsScreen({
    Key key,
    this.user,
  }) : super(key: key);
  @override
  FirstTimeTagsScreenState createState() =>
      FirstTimeTagsScreenState(user: user);
}

class FirstTimeTagsScreenState extends State<FirstTimeTagsScreen>
  ...
  Future<User> refreshProfile() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (user.profile_type == 'P') {
        return Navigator.of(context).push(MaterialPageRoute(
            builder: (context) => HomeScreen(user: mainuser)));
      }
    }
  }
              ...
              child: Column(
                ...
                children: <Widget>[
                  ...
                  ElevatedButton(
                    ...
                    onPressed: () {
                      if (tagsfinal.isNotEmpty) {
                        for (int i = 0; i < tagsfinal.length; i++) {
                          CreateUserTags(tagsfinal[i], user.id);
                        }
                      }
