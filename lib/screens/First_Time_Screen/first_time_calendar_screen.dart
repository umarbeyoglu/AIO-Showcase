...
class FirstTimeBusinessScreen extends StatefulWidget {
  final User user;
  const FirstTimeBusinessScreen({
    Key key,
    this.user,
  }) : super(key: key);
  @override
  FirstTimeBusinessScreenState createState() =>
      FirstTimeBusinessScreenState(user: user);
}

class FirstTimeBusinessScreenState extends State<FirstTimeBusinessScreen>
  ...
  Future<User> refreshProfile() async {
    ...
    if (response.statusCode == 200) {
      ...
      return Navigator.of(context).push(MaterialPageRoute(
          builder: (context) => FirstTimeMerchantScreen(user: mainuser)));
    }
  }

  bool typechoice(String type) {
    if (type == 'calendartype2') {
      return true;
    }
    if (type == 'calendartype3') {
      return true;
    }
    return false;
  }
                  ...
                  children: <Widget>[
                    ...
                    ElevatedButton(
                      ...
                      onPressed: () {
                        if (type != '') {
                          if (type1 == 'Restaurant')
                            EditUserBusinessType(user, '$type1|$type');
                          if (type1 == 'GroceryShopping')
                            EditUserBusinessType(user, '$type1');
                        }
                        ...
                        ElevatedButton(
                          ...
                          onPressed: () {
                            Navigator.of(context).push(MaterialPageRoute(
                                builder: (context) =>
                                    FirstTimeMerchantScreen(user: user)));
                          },
                        ),
                        SizedBox(
...
class EditBusinessScreenState extends State<FirstTimeBusinessScreen>
    with SingleTickerProviderStateMixin {
  final User user;
