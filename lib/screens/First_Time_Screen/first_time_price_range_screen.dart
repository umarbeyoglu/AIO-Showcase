...
class FirstTimePriceRangeScreen extends StatefulWidget {
  final User user;
  const FirstTimePriceRangeScreen({Key key, this.user,}) : super(key: key);
  @override
  FirstTimePriceRangeScreenState createState() => FirstTimePriceRangeScreenState(user:user);
}

class FirstTimePriceRangeScreenState extends State<FirstTimePriceRangeScreen> with SingleTickerProviderStateMixin{
  final User user;
  ...
  categories13(String categorieschoices) {
    ...
    if (categorieschoices == premiumst) {pricerange = '$premiumst';
      setState(() {EditUserPriceRange(user, 'premium',context);

      });
    }
    if (categorieschoices == highst) {pricerange = '$highst';
      setState(() {EditUserPriceRange(user, 'high',context); });
    }
    if (categorieschoices == standardst) {    pricerange = '$standardst';
      setState(() {EditUserPriceRange(user, 'standard',context);});
    }
    ...
    if (categorieschoices == unspecifiedst) {
      ...
      setState(() {EditUserPriceRange(user, '',context); });
    }
  }
              ...
              child: Column(
                ...
                children: <Widget>[
                        ...
                        itemBuilder: (BuildContext context){
                          return pricerangechoices.map((String categories){
                            return PopupMenuItem<String>(
                              value: categories,
                              child: Text(categories,style: TextStyle(color:colordtmaintwo,),),
                            );
                          }).toList();
                        },
                      ),
                  ...
                  Padding(
                    ...
                    child: Text(firsttimepricerangeexpst,
                      style: TextStyle(
                        color:colordtmaintwo,
                        fontSize: MediaQuery.of(context).size.height*0.025,
                        fontWeight: FontWeight.bold,
