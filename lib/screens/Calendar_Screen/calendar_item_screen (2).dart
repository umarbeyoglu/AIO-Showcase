...
class ItemScreen extends StatefulWidget {
  ...
  const ItemScreen({Key key,this.calendarowner, this.ischosing,this.user}) : super(key: key);
  @override
  ItemScreenState createState() => ItemScreenState(user:user,ischosing:ischosing,calendarowner:calendarowner);
}
...
class ItemScreenState extends State<ItemScreen> {
    ...
    return showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Text( 'Choose items to add for status list. \n \n Press -> to go next \n Press + to add item to list \n'
  ...
  void initState() {
    if(calendarowner.id == user.id){
      setState(() {
        print('ff');
        fetchPostsfuture =  FetchOwnArticlesFiltered(user.id, page,  calendarowner, false, '', '', '','CI');
      });   return;
    ...
    isuserfollowed(user.id,calendarowner.id)..then((result){
      if(user.shouldshowuserinfo(calendarowner,result) == 'followed'){
        setState(() {
          fetchPostsfuture = FetchFollowedArticlesFiltered(user.id, page,  calendarowner, false, '', '', '','CI');
        });   return;

      }
      if(user.shouldshowuserinfo(calendarowner,result) == 'public'){
        setState(() {
          fetchPostsfuture = FetchPublicArticlesFiltered(user.id, page,  calendarowner, false, '', '', '','CI');
        });   return;
  ...
  Future _loadData() async {
    if(calendarowner.id == user.id){
      FetchOwnArticlesFiltered(user.id, page,  calendarowner, false, '', '', '','CI')..then((result2){
        setState(() {
    ...
    isuserfollowed(user.id,calendarowner.id)..then((result){
      if(user.shouldshowuserinfo(calendarowner,result) == 'followed'){
        FetchFollowedArticlesFiltered(user.id, page,  calendarowner, false, '', '', '','CI')..then((result2){
          setState(() {
            for(final item2 in result2)
              posts.add(item2);
            isLoading = false;
      ...
      if(user.shouldshowuserinfo(calendarowner,result) == 'public'){
        FetchPublicArticlesFiltered(user.id, page,  calendarowner, false, '', '', '','CI')..then((result2){
          setState(() {
            for(final item2 in result2)
              posts.add(item2);
            isLoading = false;
            ...
            SliverAppBar(
              ...
              actions: [
                ...
                IconButton(
                  ...
                  color: colordtmaintwo,
                  onPressed: () {
                    Navigator.of(context).push(MaterialPageRoute(builder: (context) => TimeScreen(user: user,)));
                  },
                ),
        ...
        body:  FutureBuilder<List<Article>>(
          ...
          builder: (context, snapshotpost) {
            ...
            PageView.builder(
                ...
                physics: ClampingScrollPhysics(),       itemCount: posts.length,
                itemBuilder: (context,index){
                  initstatsitem(posts[index]);
                  return ArticleWidgetScreen(post:posts[index],ischosing:true,user:user,dayhere: '',mnyhere: '');
                }) : Center(
