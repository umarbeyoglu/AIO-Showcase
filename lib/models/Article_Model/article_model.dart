...
import 'article_tagged_user_model.dart';
...
class Article {
  ...
  Article({
    ...
    this.startdate,
    this.readtime,
    this.deliveredfromtime,
    this.deliveredtotime,
    ...
    this.detailspecs,
    this.detailrules,
    this.detailincludeds,
    this.detailcategories,
    this.detailamenities,
  ...
  factory Article.fromJSON(Map<String, dynamic> jsonMaparticle) {
    return Article(
      ...
      detailspecs: jsonMaparticle["articledetailspecs_set"] != null
          ? List<ArticleDetailSpec>.from(
              jsonMaparticle["articledetailspecs_set"]
                  .map((x) => ArticleDetailSpec.fromJSON(x)))
          : [],
      detailrules: jsonMaparticle["articledetailrules_set"] != null
          ? List<ArticleDetailRule>.from(
              jsonMaparticle["articledetailrules_set"]
                  .map((x) => ArticleDetailRule.fromJSON(x)))
          : [],
      detailincludeds: jsonMaparticle["articledetailincludeds_set"] != null
          ? List<ArticleDetailIncluded>.from(
              jsonMaparticle["articledetailincludeds_set"]
                  .map((x) => ArticleDetailIncluded.fromJSON(x)))
          : [],
      detailcategories: jsonMaparticle["articledetailcategories_set"] != null
          ? List<ArticleDetailCategory>.from(
              jsonMaparticle["articledetailcategories_set"]
                  .map((x) => ArticleDetailCategory.fromJSON(x)))
          : [],
      detailamenities: jsonMaparticle["articleamenities_set"] != null
          ? List<ArticleAmenity>.from(jsonMaparticle["articleamenities_set"]
              .map((x) => ArticleAmenity.fromJSON(x)))
          : [],
      travellocations: jsonMaparticle["articletravellocations_set"] != null
          ? List<ArticleTravelLocation>.from(
              jsonMaparticle["articletravellocations_set"]
                  .map((x) => ArticleTravelLocation.fromJSON(x)))
          : [],
      highlights: jsonMaparticle["articlehighlights_set"] != null
          ? List<ArticleHighlight>.from(jsonMaparticle["articlehighlights_set"]
              .map((x) => ArticleHighlight.fromJSON(x)))
          : [],
      activities: jsonMaparticle["articleactivities_set"] != null
          ? List<ArticleActivity>.from(jsonMaparticle["articleactivities_set"]
              .map((x) => ArticleActivity.fromJSON(x)))
          : [],
      forms: jsonMaparticle["articleforms_set"] != null
          ? List<ArticleForm>.from(jsonMaparticle["articleforms_set"]
              .map((x) => ArticleForm.fromJSON(x)))
          : [],
      checkboxes: jsonMaparticle["articlecheckboxes_set"] != null
          ? List<ArticleCheckBox>.from(jsonMaparticle["articlecheckboxes_set"]
              .map((x) => ArticleCheckBox.fromJSON(x)))
          : [],
      images: jsonMaparticle["articleimages_set"] != null
          ? List<ArticleImage>.from(jsonMaparticle["articleimages_set"]
              .map((x) => ArticleImage.fromJSON(x)))
          : [],
      sharedimages: jsonMaparticle["articlesharedimages_set"] != null
          ? List<ArticleSharedImage>.from(
              jsonMaparticle["articlesharedimages_set"]
                  .map((x) => ArticleSharedImage.fromJSON(x)))
          : [],
      sharedvideos: jsonMaparticle["articlesharedvideos_set"] != null
          ? List<ArticleSharedVideo>.from(
              jsonMaparticle["articlesharedvideos_set"]
                  .map((x) => ArticleSharedVideo.fromJSON(x)))
          : [],
      videos: jsonMaparticle["articlevideos_set"] != null
          ? List<ArticleVideo>.from(jsonMaparticle["articlevideos_set"]
              .map((x) => ArticleVideo.fromJSON(x)))
          : [],
      tags: jsonMaparticle["articletags_set"] != null
          ? List<ArticleTag>.from(jsonMaparticle["articletags_set"]
              .map((x) => ArticleTag.fromJSON(x)))
          : [],
      usertags: jsonMaparticle["articleusertags_set"] != null
          ? List<ArticleTaggedUser>.from(jsonMaparticle["articleusertags_set"]
              .map((x) => ArticleTaggedUser.fromJSON(x)))
          : [],
      choices: jsonMaparticle["articlechoices_set"] != null
          ? List<ArticleChoice>.from(jsonMaparticle["articlechoices_set"]
              .map((x) => ArticleChoice.fromJSON(x)))
          : [],
      choicecategories: jsonMaparticle["articlechoicecategories_set"] != null
          ? List<ArticleChoiceCategory>.from(
              jsonMaparticle["articlechoicecategories_set"]
                  .map((x) => ArticleChoiceCategory.fromJSON(x)))
          : [],
    );
  }
  ...
  void articleunbookmarkprocess(
      String userid, Article article1, User user, String condition) {
    if (author == user.id) {
      return;
    }
    if (author == userid) {
  ...
  bool articleliked(String userid, bool liked, bool unliked, bool condition) {
    ...
    if (unliked == true && likeresult == false) {
      return false;
    }
    if (likeresult) {
      return true;
  ...
  bool articledisliked(
      String userid, bool disliked, bool undisliked, bool condition) {
    bool like1 = false;
  ...
  void articleundislikeprocess(
      String userid, Article article1, String condition) {
  ...
  bool articlebookmarked(
      String userid, bool bookmarked, bool unbookmarked, User user) {
    if (author == user.id) {
      return true;
    }
    if (author == userid) {
    ...
    if (unbookmarked == true && bookmarkresult == false) {
      return false;
    }
    if (bookmarkresult) {
      return true;
...
class ArticleChoiceCategory {
  ...
  factory ArticleChoiceCategory.fromJSON(Map<String, dynamic> jsonMap) {
    return ArticleChoiceCategory(
      ...
      timestamp: jsonMap['timestamp'] as String,
      image: jsonMap['image'] as String,
      allchooseable: jsonMap['allchooseable'] as bool,
...
class ArticleCheckBox {
  ...
  ArticleCheckBox(
      {this.id, this.service, this.author, this.hint, this.edithint});
