import 'package:flutter_food_delivery_app/models/User_Model/user_model.dart';
...
class ReservationDeactivationMonth {
  ...
  ReservationDeactivationMonth(
      {this.id, this.enddate, this.timestamp, this.author, this.startdate});
...
class RequestInit {
  ...
  factory RequestInit.fromJSON(Map<String, dynamic> jsonMap) {
    return RequestInit(
      ...
      time: jsonMap['time'] as String,
      date: jsonMap['date'] as String,
      choices: jsonMap["requestchoices_set"] != null
          ? List<RequestItemChoice>.from(jsonMap["requestchoices_set"]
              .map((x) => RequestItemChoice.fromJSON(x)))
          : [],
      images: jsonMap["requestimages_set"] != null
          ? List<RequestImage>.from(
              jsonMap["requestimages_set"].map((x) => RequestImage.fromJSON(x)))
          : [],
      forms: jsonMap["requestforms_set"] != null
          ? List<RequestForm>.from(
              jsonMap["requestforms_set"].map((x) => RequestForm.fromJSON(x)))
          : [],
      checkboxes: jsonMap["requestcheckboxes_set"] != null
          ? List<RequestCheckBox>.from(jsonMap["requestcheckboxes_set"]
              .map((x) => RequestCheckBox.fromJSON(x)))
          : [],
    );
  }
}
